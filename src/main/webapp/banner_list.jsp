    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="reaAddModalLabel">新增轮播图</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="addPic" action="" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">图片名称:</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control " name="picName"
                                   id="reaName_add_input" placeholder="图片名称"> <span
                                class="help-block"></span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">选择图片:</label>
                        <div class="col-sm-10">
                            <input type="file" name="fileImg" id="fileImg_add_input" class="file" />
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function showPage(n) {
        $.ajax({
            url: "${pageContext.request.contextPath}/picture/showBannerByPage",
            data: "pn=" + n,
            type: "GET",
    ...
    function build_rea_table(result) {
        ...
        $.each(admins, function (adminlist, item) {
            ...
            var picNameTd = $("<td></td>").append(item.picName);
            var picFilePathTd = $("<td></td>").append($("<img width='50' height='50' />").attr("src",${pageContext.request.contextPath}item.picFilePath));
            var picUpdateTimeTd = $("<td></td>").append(getLocalTime(item.picUpdateTime));
            ...
            $("<tr></tr>")
                ...
                .append(idTd)
                .append(picNameTd)
                .append(picFilePathTd)
                .append(picUpdateTimeTd)
                .append(deleteBtnTd)
                .appendTo("#rea_table")
        });
    }
    ...
    $(document).on("click", ".delete-btn", function () {
        ...
        if (confirm("确定要删除[" + reaName + "]吗？")) {
            //2.发送AJAX请求，删除员工数据
            $.ajax({
                url: "${pageContext.request.contextPath}/picture/removeBanner",
    ...
    $("#rea_add_btn").click(
        function () {
            var uploadFile = new FormData($("#addPic")[0]);
            //uploadFile.append('picName',$("#reaName_add_input").val());
            console.log(uploadFile);
            $.ajax({
                url: "${pageContext.request.contextPath}/picture/savePicture",
                type: "POST",
                data: uploadFile,
                contentType: false,
                processData: false,
